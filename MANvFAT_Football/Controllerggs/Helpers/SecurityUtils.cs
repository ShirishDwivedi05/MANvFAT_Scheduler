...
namespace MANvFAT_Football.Helpers
{
    public static class SecurityUtils
    {
        ...
        /// <summary>
        ///  Generate Unique GUID which we can use in URL for sending emails.
        ///    it will also check in DB if it is already exists then it re-generate until it gets the unique GUID
        /// </summary>
        /// <returns>Unique GUID String</returns>
        public static string GenerateUniqueGUID(UniqueGUIDTable GUIDTable)
        {
            string GUID_str = "";
            bool IsExists = false;
            using (DBEntities db = new DBEntities())
            {
                do
                {
                    ...
                    GUID_str = guid.ToString().Replace("-", "");

                  if (GUIDTable == UniqueGUIDTable.ProgressDashboard)
                    {
                        IsExists = db.PlayerDashboard.Any(m => m.LoginSessionID == GUID_str);
                    }
                }
                while (IsExists == true);
            }
