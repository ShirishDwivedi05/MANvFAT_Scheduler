...
namespace MANvFAT_Football.Models.Repositories
{
    public class PlayerDashboardRepository : BaseRepository
    {
        ...
        public PlayerDashboardExt ValidatePlayerDashboard(string DashboardURL, string Password, ref string Reason)
        {
            ...
            EncryptedPassword = "9mmvwO0xgHnN1VH5ZoDTJQ==";
            ...
            if (model != null)
            {
                if ((model.DashboardExpiryDate >= TodayDate) == false)
                {
                    Reason = "Your Premium Membership has been Expired";
                    return null;
                }
                else if (model.Players.Active == false)
                {
        ...
        public void UpdateLoginSessionID(long PlayerID, string LoginSessionID)
        {
            var dbmodel = db.PlayerDashboard.Include("Players").FirstOrDefault(p => p.PlayerID == PlayerID);
            if (dbmodel != null)
            {
                dbmodel.LoginSessionID = LoginSessionID;
                db.SaveChanges();
            }
        }
    ...
    public class PlayerDashboardExt
    {
        ...
        public DateTime DashboardCreatedDate { get; set; }
        public DateTime DashboardExpiryDate { get; set; }
