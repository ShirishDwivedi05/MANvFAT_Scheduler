...
namespace MANvFAT_Football.Helpers
{
    public static class ErrorHandling
    {
        ...
        public static string HandleModelStateErrors(ModelStateDictionary ModelState)
        {
            StringBuilder sb = new StringBuilder();
            foreach (var item in ModelState.Values)
            {
                foreach (var item_errors in item.Errors)
                {
                    sb.Append(item_errors.ErrorMessage + "<br/>");
                }
            }
            return sb.ToString();
        }
        ...
        private static string GetErrorMessageAndLogException(Exception ex)
        {
            ...
            {
                var DbException = (DbUpdateException)ex;
                if (DbException.InnerException != null)
                {
                    errorMessage = DbException.InnerException.Message;

                    if (DbException.InnerException.InnerException != null)
                    {
                        errorMessage = DbException.InnerException.InnerException.Message;

                        if (errorMessage.ToLower().Contains("REFERENCE".ToLower()))
                        {
                            errorMessage = "You cannot delete this record. It is already in use.";
                        }
                        else if (errorMessage.ToLower().Contains("cannot insert duplicate key".ToLower()))
                        {
                            errorMessage = "Data already exists it should be unique.";
                        }
                    }
                }
                else
