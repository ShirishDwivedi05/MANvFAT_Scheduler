    ...
    public class PlayersController : BaseController
    {
        ...
        public ActionResult Details(long id)
        {
            if (id != 0)
            {
                ...
                {
                    //if logged in Coach is not coach for requested league details then restrict the user and redirect to UnAuthorizedAccess Page
                    if (model.SelectedLeagueCoachUserID != LoggedInUser.UserID)
                    {
                        return RedirectToAction("UnAuthorizedAccess", "Error");
                    }
                }
                else if (LoggedInUser.RoleID == (long)Permissions.Administrator)
                {
        ...
        {
            model.Heights = DropDownLists.GetHeights(model.HeightID);
            model.HowActives = DropDownLists.GetHowActives(model.HowActiveID);
            PlayerImagesRepository pImgRepo = new PlayerImagesRepository();
            model.PlayerImgs = pImgRepo.ReadAll(model.PlayerID, false,true);
            ViewBag.PlayerID = model.PlayerID;
            //PlayersRepository pRepo = new PlayersRepository();
            //model.PlayerWeightLogStats = pRepo.GetPlayerWeightLogStats(model.PlayerID);

            SystemSettingsRepository sysRepo = new SystemSettingsRepository();
            var sys = sysRepo.GetSystemSettings();

            string EncryptedEmail = SecurityUtils.EncryptText(model.EmailAddress);

            string _GalleryLink = sys.CurrentDomain + "/Home/MyProgressGallery?p=" + EncryptedEmail;

            model.ProgressGalleryLink = _GalleryLink;
        ...
        public ActionResult ExportToExcel()
        {
            ...
            headerRow.GetCell(17).CellStyle =
            headerRow.GetCell(18).CellStyle =
            headerRow.GetCell(19).CellStyle =
            headerRow.GetCell(20).CellStyle =
            headerRow.GetCell(21).CellStyle =
            headerRow.GetCell(22).CellStyle =
            headerRow.GetCell(23).CellStyle =
            headerRow.GetCell(24).CellStyle = CreateExcelFile_NPOI.CellStyle(ref workbook, true, true, false, null, null, null, null, null, null);
            ...
            foreach (PlayersExt p in players)
            {
                ...
                row.CreateCell(6).SetCellValue(p.SelectedLeagueName);
                row.CreateCell(7).SetCellValue(Convert.ToDouble(p.Weight, provider));
                row.CreateCell(8).SetCellValue(p.Height);
                row.CreateCell(9).SetCellValue(p.HowActiveAreYou);
                row.CreateCell(10).SetCellValue(p.PrefferredPosition);
                row.CreateCell(11).SetCellValue(Convert.ToDouble(p.BMI, provider));
                row.CreateCell(12).SetCellValue(Convert.ToDouble(p.BodyFat, provider));
                row.CreateCell(13).SetCellValue((p.Active ? "Yes" : "No"));
                row.CreateCell(14).SetCellValue((p.Featured ? "Yes" : "No"));
                row.CreateCell(15).SetCellValue(p.WorkPhone);
                row.CreateCell(16).SetCellValue(p.Landline);
                row.CreateCell(17).SetCellValue(p.Mobile);
                ...
                row.CreateCell(22).SetCellValue(p.PostCode);
                row.CreateCell(23).SetCellValue(p.Notes);
