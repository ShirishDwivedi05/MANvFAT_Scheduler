...
namespace MANvFAT_Football.Controllers
{
    public class PlayerImagesController : Controller
    {
        ...
        public string ValidatePlayer_ForProgressImage(PlayersExt player, List<PlayerImagesExt> ListOfPlayerImages, long LeagueID, bool IsFront, bool IsSide)
        {
            ...
            else if (player.SelectedLeagueID == null || player.SelectedLeagueID == 0)
            {
                Msg = "Unable to upload your Photos, because you are not assigned to any League in MANvFAT System";
            }
            else if (player.PlayerTeamID == 0)
            {
                Msg = "Unable to upload your Photos, because you are not assigned to any team in MANvFAT System";
            }
            else if (player.SelectedLeagueID != LeagueID)
            {
                Msg = "Unable to upload your Photos, because you are not assigned to this League in MANvFAT System";
            }
            else
            {
        ...
        public ActionResult SavePlayerImages(IEnumerable<HttpPostedFileBase> PlayerImages, long ParamPlayerID)
        {
            string FinalFileName = "";
            // The Name of the Upload component is "files"
            if (PlayerImages != null)
            {
                ...
                foreach (var file in PlayerImages)
                {
                    if (!(System.IO.Directory.Exists(PlayerImagesFolder)))
                    {
                        System.IO.Directory.CreateDirectory(PlayerImagesFolder);
        ...
        public ActionResult RemovePlayerImages(string[] PlayerImgs, long ParamPlayerID)
        {
            string Msg = "";
