...
namespace MANvFAT_Football.Models.Repositories
{
    public class CookiesRespository
    {
        ...
        public HttpCookie CreateDashboardLoginCookie(string EmailAddress, bool IsRememberFor2Weeks)
        {
            string CookieName = PremiumDashboard_CookieName;

            HttpCookie cookie = new HttpCookie(CookieName);

            cookie.Values["MVFProgressDashboard"] = SecurityUtils.EncryptText(EmailAddress);

            if (IsRememberFor2Weeks)
            {
        ...
        public CookieExt GetDashboardLoginCookie(Controller ctrl)
        {
            ...
            try
            {
                ...
                if (cookie != null)
                {
                    var Encrypted_EmailAddress = ctrl.Request.Cookies[CookieName].Values["MVFProgressDashboard"].ToString();
                    try
                    {
                        ...
                        {
                            LoginProgressDashboard = _EmailAddress
                        };
                    }
                    catch (Exception ex)
                    {
    ...
    public class CookieExt
    {
        ...
        public string LoginProgressDashboard { get; set; }
